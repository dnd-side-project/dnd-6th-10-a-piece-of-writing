...
//        return postsService.findAllPostsOrderById();
    ...
    @Operation(summary = "select posts containing searched content api", description = "검색된 키워드를 포함한 게시물 조회 api. request 받은 내용 기준으로 화면에서 글귀를 최신 순으로 페이지당 size개씩 조회.")
    ...
    public ResponseEntity<ResponseDto> findAllPostsBySearch(@PathVariable int page,
        ...
        return responseServiceImpl.successResult(
            SuccessCode.SELECT_POSTS_CONTATINING_CONTENT_SUCCESS, posts);
    }
